...
func (tp *TaskPlan) create(entries []map[string]any) types.Result {
	tp.mu.Lock()
	defer tp.mu.Unlock()
	...
	for _, e := range entries {
		...
		if title == "" {
			return types.Result{Output: "each task must have a non-empty 'title'", IsErr: true}
		}
		status := taskPending
	...
	tp.tasks = items
	tp.nextID = id
	return types.Result{Output: fmt.Sprintf("Created %d task(s).", len(items))}
}

func (tp *TaskPlan) add(title string) types.Result {
	if title == "" {
		return types.Result{Output: "'title' is required", IsErr: true}
	}
	tp.mu.Lock()
	defer tp.mu.Unlock()
	...
	return types.Result{Output: fmt.Sprintf("Added task [%d] %q.", id, title)}
}
...
func (tp *TaskPlan) list() types.Result {
	...
	if len(tp.tasks) == 0 {
		return types.Result{Output: "No tasks."}
	}
...
func (tp *TaskPlan) deleteAll() types.Result {
	tp.mu.Lock()
	defer tp.mu.Unlock()
	...
	return types.Tool{
		...
		Handler: func(_ context.Context, input map[string]any) types.Result {
			...
			case "create":
				...
				if !ok || len(raw) == 0 {
					return types.Result{Output: "'tasks' must be a non-empty array of {title, status?}", IsErr: true}
				}
			...
			case "update":
				...
				status, err := types.GetString(input, "status")
				if err != nil {
					return types.ErrResult(err)
				}
